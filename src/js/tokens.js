...
DJS.Lexer = Class.create(DJS.Base, {
	...
	getNumber: function( token ) {
		...
		switch( pattern ) {
			...
			case "9E+9":
			case "9E-9":
			case "9E9":			return new DJS.Token( DJS.Token.FLOAT_LITERAL, token, this.startOfParse, this.parseCharIndex - token.length, this.lineCharIndex - token.length, this.lineNumber );
			
			case "9":			return new DJS.Token( DJS.Token.FLOAT_LITERAL, token, this.startOfParse, this.parseCharIndex - token.length, this.lineCharIndex - token.length, this.lineNumber );
								
			case "9xF":			if( token.charAt( 0 ) == "0" ) {
